...
class BilibiliAuto:
    ...
    def search_videos(self, keyword, count=None, page_size=20):
        """
        搜索视频

        Args:
            keyword: 搜索关键词
            count: 要搜索的视频数量，如果为None则只搜索第一页
            page_size: 每页结果数
        ...
        try:
            ...
            while True:
                params = {
                    ...
                    "page": page,
                    "page_size": page_size,
                    "order": "pubdate",  # 综合排序 totalrank 最新发布排序 pubdate
                    "platform": "pc"
                }
                ...
                if data["code"] == 0:
                    ...
                    if not videos:  # 没有更多结果
                        break

                    all_videos.extend(videos)
                    print(
                        f"第 {page} 页找到 {len(videos)} 个视频，累计 {len(all_videos)} 个")

                    # 如果指定了数量且已达到要求，则停止搜索
                    if count and len(all_videos) >= count:
